...
import 'package:coper/view/shop/home/model/category_model.dart';
import 'package:coper/view/shop/home/service/home_shop_service.dart';
import 'package:coper/view/shop/home/view_model/shop_home_view_model.dart';
...
class ShopHomeView extends StatelessWidget with BaseState {
  ...
  Widget build(BuildContext context) {
    return BaseView<ShopHomeViewModel>(
      ...
      onModelReady: (model) {
        model.setContext(
            context as ReactiveContext); // as ReactiveContext EKLEME YAPILDI MY
        model.init();
      },
      builder: (BuildContext context, ShopHomeViewModel viewModel) => Scaffold(
        body: ListView(
          ...
          children: [
            ...
            Observer(
                builder: (_) =>
                    _categoriesProducts(context, viewModel.categories)),
            context.emptySizedHeightBoxLow,
            PaddingNormal(
                child:
                    TitleText(LocaleKeys.home_latest.tr(), context: context)),
            Observer(
                builder: (_) => _latestProducts(context, viewModel.latests)),
            context.emptySizedHeightBoxLow,
            Observer(
                builder: (_) => _bottomProducts(viewModel.products, context))
          ],
        ),
      ),
    );
    ...
    return PaddingNormal(
      ...
      children: [
        ...
        TextButton.icon(
            ...
            label: Text(LocaleKeys.home_seeAll.tr(),
  ...
  Widget _categoriesProducts(BuildContext context, List<CategoryModel> items) {
    return items.isEmpty
        ? SizedBox(
            ...
            child: LottieBuilder.asset(imageConstants.lottiePath.shopBag))
        : PaddingNormal(
            child: SizedBox(
                height: context.dynamicHeight(0.15),
                child: CategoriesRow(items: items.take(4).toList())),
          );
  }
  ...
  Widget _latestProducts(BuildContext context, List<LatestModel> items) {
    return items.isEmpty
        ? SizedBox(
            height: context.dynamicHeight(0.3),
            child: LottieBuilder.asset(imageConstants.lottiePath.shopCar))
        : SizedBox(
            height: context.dynamicHeight(0.3),
    ...
    return PaddingNormal(
      child: GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 3, mainAxisExtent: context.dynamicHeight(0.2)),
